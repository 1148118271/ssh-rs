...
where
    S: Read + Write + Unpin,
{
    pub(crate) async fn open(channel: Channel<S>, tv: TerminalSize) -> SshResult<Self> {
        // shell 形式需要一个伪终端
        let mut channel_shell = ChannelShell(channel);
        channel_shell.request_pty(tv).await?;
        channel_shell.get_shell().await?;
        Ok(channel_shell)
    }

    async fn request_pty(&mut self, tv: TerminalSize) -> SshResult<()> {
        let tvs = tv.fetch();
        println!("tvs {:?}", tvs);
    ...
    async fn get_shell(&mut self) -> SshResult<()> {
        let mut data = Data::new();
        data.put_u8(ssh_msg_code::SSH_MSG_CHANNEL_REQUEST)
            .put_u32(self.server_channel_no)
    ...
    ///
    pub async fn read(&mut self) -> SshResult<Vec<u8>> {
        let mut out = self.recv().await?;
        while let Ok(Some(mut data)) = self.try_recv().await {
            out.append(&mut data)
        }
        Ok(out)
    }
    ...
    ///
    pub async fn write(&mut self, buf: &[u8]) -> SshResult<()> {
