...
impl Client {
    ...
    pub async fn do_auth_async<S>(&mut self, stream: &mut S, digest: &mut Digest) -> SshResult<()>
    where
        S: async_std::io::Read + Unpin + async_std::io::Write,
    {
        ...
        let mut tried_public_key = false;
        loop {
            let mut data = Data::unpack(SecPacket::from_stream_async(stream, self).await?)?;
            let message_code = data.get_u8();
            match message_code {
                ssh_msg_code::SSH_MSG_SERVICE_ACCEPT => {
                    if self.config.auth.key_pair.is_none() {
                        ...
                        // if no private key specified
                        // just try password auth
                        self.password_authentication_async(stream).await?
                    } else {
                        // if private key was provided
                        // use public key auth first, then fallback to password auth
                        self.public_key_authentication_async(stream).await?
                    }
                }
                ssh_msg_code::SSH_MSG_USERAUTH_FAILURE => {
                    if !tried_public_key {
                ...
                ssh_msg_code::SSH_MSG_USERAUTH_PK_OK => {
                    log::info!("user auth support this algorithm.");
                    self.public_key_signature_async(stream, digest).await?
                }
                ssh_msg_code::SSH_MSG_USERAUTH_SUCCESS => {
                    log::info!("user auth successful.");
                    return Ok(());
    ...
    async fn password_authentication_async<S>(&mut self, stream: &mut S) -> SshResult<()>
    where
    ...
    async fn public_key_authentication_async<S>(&mut self, stream: &mut S) -> SshResult<()>
    where
        S: Unpin + async_std::io::Write,
    {
        let data = {
    ...
    pub(crate) async fn public_key_signature_async<S>(
        &mut self,
        stream: &mut S,
        digest: &Digest,
    ) -> SshResult<()>
